...
contract MAINNET_DeployVault is Script {
    uint256 privateKey;
    uint16 fees;
    ...
    function run() external {
        ...
        mgr = vm.envAddress("VAULTMANGER_ADDRESS");
        ...
        AsyncVault(address(proxyUSDC)).transferOwnership(mgr);
        AsyncVault(address(proxyWETH)).transferOwnership(mgr);
        ...
        // Mainnet
        // source .env && forge clean && forge script script/mainnet_deploy.s.sol:MAINNET_DeployVault --ffi --chain-id 1 --optimizer-runs 10000 --verifier-url ${VERIFIER_URL} --etherscan-api-key ${ETHERSCAN_API_KEY} --verify #--broadcast
        // Sepolia
        // source .env && forge clean && forge script script/mainnet_deploy.s.sol:MAINNET_DeploVault --ffi --chain-id 534351 --optimizer-runs 10000 --verifier-url ${VERIFIER_URL_SEPOLIA} --etherscan-api-key ${ETHERSCAN_API_KEY} --verify #--broadcast
    }
}
